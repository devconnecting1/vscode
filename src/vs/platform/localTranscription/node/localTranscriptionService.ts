...
// Inline stubs for deleted foundry-local-sdk
type FoundryLocalManager = { readonly catalog: { getModel(id: string): Promise<IModel> }; dispose(): Promise<void> };
type IModel = {
	readonly isCached: boolean;
	download(progress?: (percent: number) => void, signal?: AbortSignal): Promise<void>;
	load(): Promise<void>;
	createAudioClient(): { settings: { language?: string }; createLiveTranscriptionSession(): LiveAudioTranscriptionSession };
};
type LiveAudioTranscriptionSession = {
	readonly settings: { sampleRate?: number; channels?: number; bitsPerSample?: number; language?: string };
	start(): Promise<void>;
	stop(): Promise<void>;
	dispose(): Promise<void>;
	append(chunk: Uint8Array): Promise<void>;
	getStream(): AsyncIterable<LiveAudioTranscriptionResponse>;
};
type LiveAudioTranscriptionResponse = {
	readonly is_final?: boolean;
	readonly start_time?: number;
	readonly end_time?: number;
	readonly content?: readonly { readonly type?: string; readonly text?: string; readonly transcript?: string }[];
};
type FoundryLocal = { readonly FoundryLocalManager: { createAsync(opts: { appName: string; modelCacheDir: string; logLevel?: string }): Promise<FoundryLocalManager> } };
...
// (type stubs for foundry-local-sdk are defined above, after imports)
	...
	private async _ensureModel(cacheDir: string, modelId: string): Promise<IModel> {
		...
		this._modelPromise = (async () => {
			try {
				...
				if (!this._sdk) {
					throw new Error('foundry-local-sdk is not available');
				}
				if (!this._manager) {
